import sqlite3, time

# Intro
print("""____________________________________________
|=Weighted Choices=========================|
|================v1.1.0-Stable=============|""")

# Setup Database
r_time = round(time.time())
save_name = "weights_"+str(r_time)+".db" # Make Filename
cw_lib = sqlite3.connect(save_name)
cur = cw_lib.cursor()
existing_combos = []
cur.execute("CREATE TABLE IF NOT EXISTS weights (choice TEXT, weight REAL)") # Create Table
cw_lib.commit() # Commit Changes

def null():
    five = 2+2
    five.bit_count()

def update_data(a,b):
    global existing_combos
    print(a + " or " + b)
    existing_combos.append(a)
    print("Press [1] for " + a + ", and [2] for " + b)
    inp = input(">>> ")
    print()
    try:
        inp = int(inp)
        if inp == 1:
            cur.execute("UPDATE weights SET weight = weight + 0.1 WHERE choice = ?", (a,))
            cur.execute("UPDATE weights SET weight = weight - 0.1 WHERE choice = ?", (b,))
            cw_lib.commit()
        elif inp == 2:
            cur.execute("UPDATE weights SET weight = weight - 0.1 WHERE choice = ?", (a,))
            cur.execute("UPDATE weights SET weight = weight + 0.1 WHERE choice = ?", (b,))
            cw_lib.commit()
    except ValueError:
        print("Invalid Input, choices added at random.")
        cur.execute("UPDATE weights SET weight = weight + 0.1 WHERE choice = ?", (a,))
        cur.execute("UPDATE weights SET weight = weight - 0.1 WHERE choice = ?", (b,))
        cw_lib.commit()

def get_weights(choices):
    global existing_combos
    # Create Base Data
    for i in choices:
        cur.execute("INSERT OR IGNORE INTO weights VALUES (?, ?)", (i, "0.0"))
    cw_lib.commit()
    # Start the data collection processes.
    for idx_i, i in enumerate(choices):
        for idx_j, j in enumerate(choices):
            if idx_j <= idx_i:
                continue  # Skip duplicates and self-comparisons
            update_data(i, j)

def wc(item_list:list):
    print("|____Press [ENTER] to begin!_______________|")
    input()
    breakfast = ["Pancakes", "Bacon", "Eggs", "Grits", "Waffles", "Bagels", "Sausage"]
    os = ["MacOS", "Windows", "Linux"]
    get_weights(item_list)
    #print("\nSuccessfully Completed!\nOutput weights saved to "+save_name)
    cur.execute("SELECT * FROM weights ORDER BY weight DESC;")
    cw_lib.commit()
    print("\n\n|=========================================|\n|=Your Results:===========================|\n|=Rank-Item-Weight========================|")
    output_data = []
    count = 0
    for row in cur.execute("SELECT * FROM weights"):
        output_data.append(row)
    for r in output_data:
        line = ""
        count += 1
        for n in r:
            try:
                num = round(n,2)
                line += str(num)
            except TypeError:
                line += "| " + str(count) + " " + n + " "
        needed_space = 42-len(line)
        spaces = ""
        for i in range(0,needed_space):
            spaces += " "
        print(line+spaces+"|")

applications = [
    "Affinity", "Audacity", "balenaEtcher", "Basic System Info", "Bluetooth Inspector",
    "Claude", "ClipGrab", "CrystalFetch", "Discord", "Disk Inspector", "Docker",
    "DumpMedia Any Music Converter", "Encrypto", "eqMac", "eul", "FedoraMediaWriter",
    "FileZilla", "GarageBand", "GitHub Desktop", "Google Earth Pro", "HashMaker",
    "iMazing 2", "JetBrains Toolbox", "KeepingYouAwake", "Launch", "LazyWeather",
    "LibreOffice", "Lite XL", "Magic Disk Benchmark", "Meshtastic", "MindNode Next",
    "Minecraft", "Mixxx", "Mu Editor", "MusicBrainz Picard", "NetSpot", "Nugget",
    "Ollama", "One Thing", "Opera", "Ora", "Packages", "Raspberry Pi Imager",
    "Recoverit", "Remote", "Safari", "ScreenSaver", "Serato DJ Lite", "Shazam",
    "Silicio", "Speedtest", "Spotify", "Steam", "Table Tool", "Tailscale", "TempBox",
    "TeraCopy", "Think Dirty", "Thonny", "VLC", "Whisky", "WiFiman", "Zenmap", "zoom.us"
]

wc(applications)